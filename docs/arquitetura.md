...
### 10.2. Problemas identificados e corrigidos durante os testes do Workflow definitivo

Ao testar o Workflow completo pela primeira vez (16 tasks), três problemas reais foram identificados e corrigidos:

**1. `DELTA_SOURCE_IGNORE_DELETE` em `bronze_vendas.py`:** o Structured Streaming, por padrão, não suporta ler incrementalmente de uma fonte onde ocorreram exclusões (`DELETE`). Como `expurgo_raw.py` remove dados de `raw.vendas` após 48h, a leitura streaming de `bronze_vendas.py` (que lê dessa mesma tabela) passou a falhar assim que o expurgo removeu alguma partição processada. Corrigido com `.option("ignoreDeletes", "true")` — seguro neste caso, pois a Bronze já copiou os dados antes de qualquer exclusão ocorrer na Raw (ver seção 5.2 para detalhamento completo).

**2. Escopo de variáveis em `silver_lojas.py`:** variáveis auxiliares (`caracteres_com_acento`, `caracteres_sem_acento`) definidas em uma célula separada durante o desenvolvimento interativo não estavam presentes na célula que efetivamente as utilizava, causando `NameError` ao rodar o notebook via Job (execução completa, do início ao fim, diferente da execução célula-a-célula usada durante o desenvolvimento). Corrigido tornando a célula de transformação autossuficiente, com as variáveis definidas nela mesma — eliminando dependência de ordem/estado entre células, prática recomendada para notebooks que rodarão de forma não interativa.

**3. Condição de corrida (race condition) entre o job auxiliar temporário e o Workflow definitivo:** durante um período de sobreposição em que ambos os jobs permaneceram ativos com o mesmo horário de agendamento (06:00), a proteção contra sobrescrita do simulador (`forcar_recriacao`, baseada em checar `dbutils.fs.ls(caminho)` antes de gravar) revelou uma falha de concorrência: como a verificação "o arquivo já existe?" e a escrita não são uma operação atômica, duas execuções simultâneas do mesmo notebook (uma do job auxiliar, outra do Workflow definitivo) podem ambas checar "não existe" antes que qualquer uma termine de escrever, resultando em duplicação de dados. Isso se manifestou nas vendas da Argentina de um dia específico (menor volume, portanto execução mais rápida, aumentando a chance de as duas execuções caírem na mesma janela de vulnerabilidade) — 1.000 linhas em vez das 500 esperadas, propagadas por Raw, Bronze e Silver até serem identificadas e corrigidas manualmente (remoção de 500 registros duplicados via `DELETE` filtrado por `venda_id`, seguida de recálculo completo da Gold).

**Causa raiz e correção definitiva:** o problema não estava no Autoloader (que processou corretamente os dois arquivos físicos distintos que lhe foram apresentados) nem na lógica de negócio do simulador — estava na manutenção simultânea de duas fontes de agendamento automatizado para a mesma responsabilidade. A correção definitiva foi pausar o job auxiliar temporário assim que o Workflow definitivo foi validado como funcional, eliminando a possibilidade de execução concorrente. Esse episódio reforça, com evidência concreta, a importância de descomissionar soluções-ponte temporárias assim que a solução definitiva estiver operacional — mantê-las "por segurança" pode, paradoxalmente, introduzir os próprios riscos que se buscava evitar.

### 10.3. Segundo episódio: duplicação por reprocessamento de streaming, backfill em batch, e reflexão sobre streaming vs. batch

**O que aconteceu:** durante o ciclo de correção dos bugs de streaming (`ignoreDeletes`/`ignoreChanges`, seção 5.2), os notebooks `bronze_vendas.py` e `silver_fato_vendas.py` foram executados manualmente em sucessão rápida diversas vezes para validar cada correção. Uma investigação de rotina revelou que o dia 27/07 estava com **2x** as linhas esperadas na Bronze e **3x** na Silver — apesar de `raw.vendas` mostrar, em seu histórico de transações, apenas uma única ingestão desse dia. A causa mais provável: uma execução interrompida ou uma corrida entre execuções concorrentes do streaming fez com que o mesmo commit da fonte fosse processado mais de uma vez antes do checkpoint confirmar definitivamente o progresso.

**A limitação descoberta ao corrigir:** o `DELETE` das linhas duplicadas em Bronze/Silver não foi suficiente para permitir um simples "rodar de novo" — o checkpoint do Structured Streaming já havia marcado aquele commit da fonte como processado, e não reprocessaria os dados novamente. Isso exigiu uma técnica de **backfill manual em batch**: ler o recorte específico da Raw via `spark.read` (não `spark.readStream`), reaplicar manualmente toda a lógica de transformação (casting, padronização, JOINs efetivos por data com dimensões e câmbio) e gravar via `.write.mode("append")`, contornando completamente o mecanismo de checkpoint, sem risco de duplicar outros dias já corretos.

**Efeito colateral identificado durante o backfill:** ao investigar a Gold, notou-se que o dia 28/07 tinha `valor_total_usd` nulo mesmo com a cotação de câmbio daquele dia já existindo em `dim_cambio`. Causa: a Bronze/Silver de vendas processou o dia 28 (09h02) **antes** de `silver_cambio.py` atualizar a cotação daquele dia (12h16) — o JOIN não encontrou correspondência no momento do processamento, e por ser um `append` (não um `MERGE` reavaliado), o valor nulo ficou gravado permanentemente. Corrigido com um segundo backfill, desta vez via `MERGE` (atualizando `cambio_usado`/`valor_total_usd` apenas para as linhas afetadas).

**Reflexão arquitetural: streaming era necessário para Bronze e Silver?** Não. Diferente da Raw (onde o Autoloader se justifica plenamente, pois rastreia arquivos chegando em uma pasta), Bronze e Silver leem de **tabelas Delta já existentes**, não de arquivos — um cenário onde `readStream` adiciona a complexidade de gerenciamento de checkpoint (e, por extensão, todos os bugs desta e da seção anterior) sem benefício proporcional, dado o baixo volume diário do projeto (~13.500 linhas). Uma alternativa mais simples — leitura batch (`spark.read`) combinada com `MERGE INTO` por chave (`venda_id`) — teria oferecido **idempotência natural**: reexecutar o mesmo notebook não duplicaria nada, e reprocessamentos (como o do câmbio tardio) se resolveriam automaticamente na execução seguinte, sem necessidade de backfills manuais. Streaming é a escolha correta para cenários de alto volume ou baixa latência; para este projeto (batch diário, baixo volume), ele trouxe custo sem o benefício correspondente. Fica registrada como uma possível refatoração futura, a ser avaliada caso a estabilidade atual (pós-correções) se mostre insuficiente na operação contínua.

**Efeito colateral do próprio processo de correção (o backfill duplicou a si mesmo):** ao corrigir o episódio acima, o dia 27/07 foi inserido manualmente tanto na Bronze quanto na Silver (via `append` em batch). Como a Silver possui um streaming lendo incrementalmente da Bronze, a próxima execução do Workflow interpretou o `append` manual na Bronze como um commit novo e legítimo a processar — reintroduzindo o dia 27/07 na Silver pela segunda vez. **Lição:** ao fazer backfill de uma camada com um streaming "rio abaixo" dependente dela, popular as duas camadas manualmente em paralelo cria esse risco; o caminho mais seguro é fazer backfill apenas na camada de origem e deixar o streaming subsequente processar naturalmente o novo commit.

**Um terceiro problema técnico durante a correção (avaliação preguiçosa do Spark):** ao tentar deduplicar a Silver com `df.filter(...).dropDuplicates(...)` seguido imediatamente de um `DELETE` e depois um `.write` do mesmo DataFrame, o resultado gravado foi **zero linhas**. Causa: DataFrames Spark são **avaliados de forma preguiçosa** (*lazy evaluation*) — a transformação só é executada de fato no momento da ação final (`.write`), não no momento em que o código é escrito. Como o `DELETE` já havia limpado a tabela de origem antes do `.write` ser avaliado, o Spark releu a fonte (já vazia) na hora de escrever. **Correção:** materializar o resultado fisicamente em uma tabela de staging (`.write.mode("overwrite").saveAsTable(...)`) **antes** de qualquer operação destrutiva na tabela original, garantindo que os dados intermediários não dependam mais da tabela que está prestes a ser alterada.

**Validação final:** após todas as correções, o Workflow completo (16 tasks) foi executado do início ao fim sem qualquer intervenção manual, mantendo os totais estáveis (Bronze e Silver em 108.000 linhas, zero duplicatas) — confirmando que as correções (`ignoreChanges` em `bronze_vendas.py` e `silver_fato_vendas.py`) resolvem o problema de forma definitiva em execução real de produção, não apenas em testes isolados.
